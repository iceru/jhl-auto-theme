<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    <div
        class="relative pt-16 before:absolute before:top-0 before:left-0 before:h-px before:w-6 before:bg-zinc-950 after:absolute after:top-0 after:right-0 after:left-8 after:h-px after:bg-zinc-950/10">
        <div class="relative lg:-mx-4 lg:flex lg:justify-end">
            <div class="pt-10 lg:w-2/3 lg:flex-none lg:px-4 lg:pt-0">
                <h2 class="text-2xl font-semibold text-zinc-950"><a href="<?php the_permalink(); ?>"
                        class="!no-underline"><?php the_title(); ?></a></h2>
                <dl class="lg:absolute lg:top-0 lg:left-0 lg:w-1/3 lg:px-4">
                    ...
                    <dd class="absolute top-0 left-0 text-sm text-zinc-950 lg:static">
                        <time datetime="<?php echo get_the_date('c'); ?>" itemprop="datePublished"
                            class="text-sm text-zinc-700"><?php echo get_the_date(); ?></time>
                    </dd>
                ...
                <a class="!no-underline mt-8 inline-flex rounded-full bg-zinc-950 px-4 py-1.5 text-sm font-semibold text-white transition hover:bg-zinc-800"
                    aria-label="<?php echo esc_attr(sprintf(__('Baca Selengkapnya: %s', 'tailpress'), get_the_title())); ?>"
                    href="<?php the_permalink(); ?>">
                    <?php _e('Baca Selengkapnya', 'tailpress'); ?>
                </a>
            </div>
        </div>
</article>